...
#
# Copyright (c) 2015 ShataCloud, Inc.
...
$(LIBUV_DIR): $(call marker,$(TMP))
	@rm -rf $(LIBUV_ROOT)
	@wget -O $(TMP)$(LIBUV_SHA).zip https://github.com/libuv/libuv/archive/$(LIBUV_SHA).zip
...
$(OPENSSL_DIR): $(call marker,$(TMP))
	@rm -rf $(OPENSSL_ROOT)
	@wget -O $(TMP)openssl-$(OPENSSL_VERSION).tar.gz ftp://ftp.openssl.org/source/openssl-$(OPENSSL_VERSION).tar.gz
...
DESCRIPTION="A reference implementation for ShataCloud's Keyless SSL server"
